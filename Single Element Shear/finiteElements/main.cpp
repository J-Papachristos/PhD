...
int main(int argc, char const *argv[]) {
    ...
    // Define Interior Force Vector
    double *Fint = (double *) malloc(nHexDOFs * elemNodes * sizeof(double));
            ...
            for (int elem = 0; elem < nElems; elem++) {
                ...
                for (int i = 0; i < elemNodes; i++) {
                    ...
                    for (int j = 0; j < elemNodes; j++) {
                        ...
                        // Add Internal Forces
                        ((double *) b_free->x)[row_tmp + DOF_ux] -= Fint[(i * nHexDOFs) + DOF_ux];
                        ((double *) b_free->x)[row_tmp + DOF_uy] -= Fint[(i * nHexDOFs) + DOF_uy];
                        ((double *) b_free->x)[row_tmp + DOF_uz] -= Fint[(i * nHexDOFs) + DOF_uz];
                    }
    ...
    for (int iKsi = 0; iKsi < GQ_POINTS; iKsi++) {
        for (int iEta = 0; iEta < GQ_POINTS; iEta++) {
            for (int iZeta = 0; iZeta < GQ_POINTS; iZeta++) {
                ...
                //                 // Maybe doesnt access elements as it should, needs to make K_NL sym/pos def
            ...
            for (int iZeta = 0; iZeta < GQ_POINTS; iZeta++) {
                ...
                for (int i = 0; i < nHexDOFs * elemNodes; i++) {
                    for (int j = 0; j < directions; j++) {
                        Fint[i] += elem->B_L[j][i] * elem->S2_v[j] * elem->detJ * weight;
                    }
                }
            }
        }
